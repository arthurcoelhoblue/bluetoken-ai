...
import { RefreshCw, Search, Eye, Calendar, Building2, Zap, AlertCircle, CheckCircle, Clock, XCircle } from 'lucide-react';
                ...
                <ScrollArea className="h-[400px]">
                  {eventDetails.leadContact ? (
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      Nenhum contato vinculado a este evento
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
              ...
              <TabsContent value="classificacao" className="mt-4">
                <ScrollArea className="h-[400px]">
                  {eventDetails.classification ? (
                      ...
                      <CardContent className="space-y-2">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="font-medium">{eventDetails.classification.icp}</p>
                          </div>
                          <div>
                            <Label className="text-xs text-muted-foreground">Temperatura</Label>
                            <p className="font-medium">{eventDetails.classification.temperatura}</p>
                          </div>
                          <div>
                            <Label className="text-xs text-muted-foreground">Persona</Label>
                            <p className="font-medium">{eventDetails.classification.persona || '-'}</p>
                          </div>
                          <div>
                            <Label className="text-xs text-muted-foreground">Prioridade</Label>
                            <p className="font-medium">P{eventDetails.classification.prioridade}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <div className="text-center py-8 text-muted-foreground">
                      Nenhuma classificação vinculada a este evento
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
              ...
              <TabsContent value="logs" className="mt-4">
                <ScrollArea className="h-[400px]">
                  {eventDetails.event.sgt_event_logs && eventDetails.event.sgt_event_logs.length > 0 ? (
                    <div className="space-y-3">
                      {eventDetails.event.sgt_event_logs.map((log, index) => (
                        <Card key={index}>
                          <CardContent className="pt-4">
                            ...
                            {log.erro_stack && (
                              <pre className="mt-2 text-xs bg-destructive/10 text-destructive p-2 rounded overflow-x-auto">
                                {log.erro_stack}
                              </pre>
                            )}
                          </CardContent>
