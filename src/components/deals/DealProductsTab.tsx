      ...
      <div className="space-y-3 p-3 border border-border rounded-lg">
        ...
        {catalog.length > 0 ? (
          <>
                ...
                <SelectContent>
                  {catalog.map(c => (
                    <SelectItem key={c.id} value={c.id}>
                      {c.nome} — {formatBRL(c.preco_unitario)}/{FREQ_LABELS[c.frequencia_cobranca] || c.frequencia_cobranca}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Button size="sm" className="w-full h-8 text-xs" onClick={handleAdd} disabled={!selectedItem || !preco || addProduct.isPending}>
              Adicionar
            </Button>
          </>
        ) : (
          <p className="text-xs text-muted-foreground">Nenhum produto no catálogo para esta empresa. Cadastre produtos em Configurações → Produtos.</p>
        )}
      </div>
