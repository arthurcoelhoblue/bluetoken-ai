...
serve(async (req) => {
  ...
  try {
    ...
    if (takeovers && takeovers.length >= 3) {
      ...
      for (const t of takeovers) { const motivo = t.motivo || 'SEM_MOTIVO'; byMotivo[motivo] = (byMotivo[motivo] || 0) + 1; }
      for (const [motivo, count] of Object.entries(byMotivo)) {
        if (count >= 3) {
          ...
          if (!(await checkDuplicate(supabase, hash))) {
            learnings.push({ empresa, tipo: 'PADRAO_TAKEOVER', categoria: 'conversacao', titulo: `Takeover recorrente: ${motivo}`, descricao: `Vendedores assumiram conversas ${count}x pelo motivo "${motivo}" nas últimas ${periodo_horas}h.`, dados: { motivo, count, periodo_horas }, confianca: Math.min(0.95, 0.5 + count * 0.1), hash_titulo: hash });
          }
        }
      }
    }
    ...
    if (perdas && perdas.length >= 3) {
      ...
      for (const [cat, count] of Object.entries(byCategoria)) {
        ...
        if (pct >= 0.4 && count >= 3) {
          ...
          if (!(await checkDuplicate(supabase, hash))) {
            learnings.push({ empresa, tipo: 'PADRAO_PERDA', categoria: 'pipeline', titulo: `${Math.round(pct * 100)}% das perdas por "${cat}"`, descricao: `${count} de ${total} deals perdidos tiveram categoria "${cat}".`, dados: { categoria: cat, count, total, pct: Math.round(pct * 100) }, confianca: Math.min(0.95, pct + 0.1), hash_titulo: hash });
          }
        }
      }
    }
    ...
    if (corrections && corrections.length >= 5) {
      ...
      if (tempCorrections >= 3) {
        ...
        if (!(await checkDuplicate(supabase, hash))) {
          learnings.push({ empresa, tipo: 'CORRECAO_CLASSIFICACAO', categoria: 'classificacao', titulo: `${tempCorrections} correções de temperatura recentes`, descricao: `Humanos corrigiram a temperatura da IA ${tempCorrections}x.`, dados: { tempCorrections, total: corrections.length }, confianca: Math.min(0.9, 0.5 + tempCorrections * 0.05), hash_titulo: hash });
        }
      }
    }
    ...
    const { data: hotIntents } = await supabase.from('lead_message_intents').select('lead_id, empresa, created_at').eq('empresa', empresa).eq('intent', 'INTERESSE_COMPRA').lt('created_at', fourHoursAgo).gte('created_at', since).limit(20);
    if (hotIntents) {
      for (const intent of hotIntents) {
        const { data: followup } = await supabase.from('lead_messages').select('id').eq('lead_id', intent.lead_id).eq('direcao', 'OUTBOUND').gt('created_at', intent.created_at).limit(1);
        if (!followup || followup.length === 0) {
          const hash = `hot_nofollowup_${intent.lead_id}`;
          if (!(await checkDuplicate(supabase, hash))) {
            learnings.push({ empresa, tipo: 'ALERTA_CRITICO', categoria: 'conversacao', titulo: `Lead quente sem followup há 4h+`, descricao: `Lead demonstrou INTERESSE_COMPRA mas não recebeu followup.`, dados: { lead_id: intent.lead_id }, confianca: 0.9, hash_titulo: hash });
          }
        }
      }
    }
    ...
    if (lostDeals && lostDeals.length >= 5) {
      ...
      for (const deal of lostDeals.slice(0, 20)) {
        const { data: activities } = await supabase.from('deal_activities').select('tipo, created_at').eq('deal_id', deal.id).order('created_at', { ascending: true }).limit(20);
        if (activities && activities.length >= 2) timelines.push({ dealId: deal.id, titulo: deal.titulo, events: activities.map(a => a.tipo) });
      }

      if (timelines.length >= 5) {
        try {
          const aiResult = await callAI({
            system: 'Analista de dados que identifica padrões em sequências de eventos de CRM. Retorne APENAS JSON válido sem markdown.',
            prompt: `Analise timelines de deals PERDIDOS e identifique subsequências de 2-4 eventos que aparecem em 50%+ dos casos.\nTimelines:\n${timelines.map((t, i) => `${i + 1}. ${t.events.join(' → ')}`).join('\n')}\n\nJSON: {"sequences":[{"events":["string"],"match_pct":number,"window_days":number,"description":"string PT-BR"}]}`,
          ...
          for (const seq of (args.sequences || [])) {
            if (seq.match_pct >= 50 && seq.events.length >= 2) {
              ...
              if (!(await checkDuplicate(supabase, hash))) {
                learnings.push({ empresa, tipo: 'SEQUENCIA_PERDA', categoria: 'sequencia', titulo: `Sequência de perda: ${seq.events.join(' → ')}`, descricao: seq.description, dados: { timelines_analyzed: timelines.length }, confianca: Math.min(0.95, seq.match_pct / 100), sequencia_eventos: seq.events, sequencia_match_pct: seq.match_pct, sequencia_janela_dias: seq.window_days, hash_titulo: hash });
              }
            }
          }
    ...
    const { data: churnLeads } = await supabase.from('contacts').select('legacy_lead_id').eq('empresa', empresa).eq('opt_out', true).gte('opt_out_em', ninetyDaysAgo).limit(20);
    if (churnLeads && churnLeads.length >= 3) {
      ...
      for (const lead of churnLeads) {
        ...
        const { data: intents } = await supabase.from('lead_message_intents').select('intent, created_at').eq('lead_id', lead.legacy_lead_id).order('created_at', { ascending: true }).limit(15);
        if (intents && intents.length >= 2) churnTimelines.push({ leadId: lead.legacy_lead_id, intents: intents.map(i => i.intent) });
      }
      ...
      if (churnTimelines.length >= 3) {
        try {
          const aiResult = await callAI({
            ...
            prompt: `Analise timelines de leads que fizeram OPT-OUT. Identifique subsequências de 2-4 intents que precedem cancelamento em 50%+ dos casos.\nTimelines:\n${churnTimelines.map((t, i) => `${i + 1}. ${t.intents.join(' → ')}`).join('\n')}\n\nJSON: {"sequences":[{"events":["string"],"match_pct":number,"window_days":number,"description":"string PT-BR"}]}`,
            functionName: 'amelia-learn',
            empresa,
            supabase,
          });
          ...
          for (const seq of (args.sequences || [])) {
            if (seq.match_pct >= 50 && seq.events.length >= 2) {
              ...
              if (!(await checkDuplicate(supabase, hash))) {
                learnings.push({ empresa, tipo: 'SEQUENCIA_CHURN', categoria: 'sequencia', titulo: `Sequência de churn: ${seq.events.join(' → ')}`, descricao: seq.description, dados: { timelines_analyzed: churnTimelines.length }, confianca: Math.min(0.95, seq.match_pct / 100), sequencia_eventos: seq.events, sequencia_match_pct: seq.match_pct, sequencia_janela_dias: seq.window_days, hash_titulo: hash });
              }
            }
          }
