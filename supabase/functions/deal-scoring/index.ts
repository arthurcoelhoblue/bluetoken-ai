...
serve(async (req) => {
  ...
  try {
    ...
    if (wonDeals) {
      ...
      for (const [pid, s] of Object.entries(sums)) {
        avgTicket[pid] = s.count > 0 ? s.total / s.count : 0;
      }
    }
    ...
    const { data: stageHistAvg } = await supabase
      ...
      .not('tempo_no_stage_anterior_ms', 'is', null)
      .limit(2000);
    ...
    if (stageHistAvg) {
      ...
      for (const [sid, times] of Object.entries(groups)) {
        stageAvgTime[sid] = times.reduce((a, b) => a + b, 0) / times.length;
      }
    }
    ...
    for (const deal of deals as any[]) {
      try {
        ...
        for (const a of activities ?? []) {
          ...
          else if (a.tipo === 'EMAIL') engPoints += 1;
          else engPoints += 1;
        }
        ...
        const score = Math.round(
          ...
          tempoScore * 0.20 +
          engajamento * 0.20 +
          temperaturaScore * 0.15 +
          valorScore * 0.10 +
          sentimentoScore * 0.10
        );
        const finalScore = Math.max(1, Math.min(99, score));

        const dimensoes = {
          stage_progress: Math.round(stageProgress),
          tempo_stage: Math.round(tempoScore),
          engajamento: Math.round(engajamento),
          temperatura: Math.round(temperaturaScore),
          valor_vs_ticket: Math.round(valorScore),
          sentimento: Math.round(sentimentoScore),
          dias_na_stage: Math.round(daysInStage),
