...
async function fetchZadarmaTranscript(supabase: ReturnType<typeof createServiceClient>, pbxCallId: string): Promise<string | null> {
  try {
    const resp = await fetch(`${envConfig.SUPABASE_URL}/functions/v1/zadarma-proxy`, {
      ...
      body: JSON.stringify({ action: 'get_transcript', payload: { call_id: pbxCallId } }),
    });

    if (!resp.ok) return null;

    const data = await resp.json();
    // Zadarma returns transcript text in data.transcript or data.result
    const text = data?.transcript || data?.result || '';
...
async function transcribeWithGeminiMultimodal(recordingUrl: string, empresa: string | null): Promise<string | null> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. Try Zadarma native transcript (free, fast)
    if (call.pbx_call_id) {
      transcription = await fetchZadarmaTranscript(supabase, call.pbx_call_id);
      if (transcription) {
        transcriptionSource = 'zadarma';
        log.info('Got Zadarma native transcript', { call_id, length: transcription.length });
      }
    }

    // 2. Fallback: Gemini Flash multimodal via Lovable AI Gateway
    if (!transcription && call.recording_url) {
      transcription = await transcribeWithGeminiMultimodal(call.recording_url, call.empresa);
      if (transcription) {
        transcriptionSource = 'gemini';
        log.info('Got Gemini multimodal transcript', { call_id, length: transcription.length });
      }
    }
    ...
    return new Response(JSON.stringify({ success: true, call_id, transcription_source: transcriptionSource, transcription_length: transcription.length, sentiment: analysis.sentiment, action_items_count: analysis.action_items?.length || 0 }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (err) {
