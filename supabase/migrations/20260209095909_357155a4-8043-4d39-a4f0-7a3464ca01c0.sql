...
-- RPC atômica: update_conversation_with_intent
-- Executa em uma única transação: upsert conversation_state + insert intent + ação na cadência
CREATE OR REPLACE FUNCTION public.update_conversation_with_intent(
  p_lead_id UUID,
  p_empresa TEXT,
  p_canal TEXT,
  p_intent_data JSONB,
...
BEGIN
  ...
  ON CONFLICT (lead_id, empresa) DO UPDATE SET
    ...
    framework_data = CASE 
      WHEN p_state_updates ? 'framework_data' THEN p_state_updates->'framework_data'
