...
BEGIN
  ...
  -- Get legacy_lead_id from contact (safely handle non-UUID values)
  SELECT ct.legacy_lead_id INTO v_legacy_text
  FROM contacts ct WHERE ct.id = NEW.contact_id;

  -- Try to cast to UUID; if invalid, skip cadence triggers
  BEGIN
    v_lead_id := v_legacy_text::UUID;
