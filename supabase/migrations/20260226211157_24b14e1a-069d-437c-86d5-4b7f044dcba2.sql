-- 1. Remove bluechat from channel constraint on integration_company_config
ALTER TABLE public.integration_company_config
  DROP CONSTRAINT IF EXISTS integration_company_config_channel_check;

ALTER TABLE public.integration_company_config
  ADD CONSTRAINT integration_company_config_channel_check
  CHECK (channel = ANY (ARRAY['mensageria'::text, 'meta_cloud'::text]));

-- Delete any residual bluechat rows
DELETE FROM public.integration_company_config WHERE channel = 'bluechat';

-- 2. Update legacy comment in update_conversation_with_intent function
...
BEGIN
  ...
  INSERT INTO lead_conversation_state (
    ...
    ultima_pergunta_id, ultimo_contato_em, updated_at
  ) VALUES (
  ...
  ON CONFLICT (lead_id, empresa) DO UPDATE SET
    ...
    framework_data = CASE 
      WHEN p_state_updates ? 'framework_data' THEN 
        -- MERGE: preserve existing keys and overlay new ones
